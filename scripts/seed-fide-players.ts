...
// FIDE player data for Sierra Leone Chess Federation
const fidePlayers = [
...
async function main() {
    console.log(`\n🏁 Starting FIDE player import...`)
    console.log(`📊 Total players to import: ${fidePlayers.length}\n`)
    ...
    for (const player of fidePlayers) {
        const memberId = `FIDE-${player.fideId}`
        // Use the highest available rating as the primary "rating" field
        const primaryRating = player.stdRating || player.rapidRating || player.blitzRating || 0
        const title = player.title || "None"
        const status = primaryRating > 0 ? "Active" : "Registered"

        try {
            const existing = await prisma.member.findUnique({ where: { id: memberId } })

            if (existing) {
                // Update existing member with latest FIDE data
                ...
                updated++
                console.log(`  ♻️  Updated: ${player.name} (STD: ${player.stdRating}, Rapid: ${player.rapidRating}, Blitz: ${player.blitzRating})`)
            } else {
                // Create new member
                await prisma.member.create({
                    data: {
                        ...
                        rapidRating: player.rapidRating,
                        blitzRating: player.blitzRating,
                        club: "Sierra Leone Chess Federation",
                        status: status,
                        joined: "2025",
                        fideId: player.fideId,
                        wins: 0,
                ...
                created++
                console.log(`  ✅ Created: ${player.name} (STD: ${player.stdRating}, Rapid: ${player.rapidRating}, Blitz: ${player.blitzRating})`)
            }
        } catch (err) {
            errors++
    ...
    console.log(`\n========================================`)
    console.log(`📋 FIDE Player Import Summary`)
    console.log(`========================================`)
    console.log(`  ✅ Created: ${created}`)
    console.log(`  ♻️  Updated: ${updated}`)
    console.log(`  ❌ Errors:  ${errors}`)
    console.log(`  📊 Total:   ${fidePlayers.length}`)
    console.log(`========================================\n`)
}
